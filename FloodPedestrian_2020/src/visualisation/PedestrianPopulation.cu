...
__global__ void output_pedestrians_to_TBO(xmachine_memory_agent_list* agents, glm::vec4* data1, glm::vec4* data2){
	...
	//data2[index].w = 0.0;
	//data2[index].w = (float)agents->HR_state[index]; // Added by MS28112018
	data2[index].w = (float)agents->stability_state[index]; // Added by MS01102019 / change the pedestrian color based on their stability state in floodwater
}
